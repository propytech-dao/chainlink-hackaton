...
describe("SimpleRent full Unit Tests", function () {
  let MockNFT, mockNFT
  ...
  it("should lend the MockNFT", async function () {
    ...
    await expect(await simpleRent.connect(owner).lend(nftAddress, tokenID, rentValue, mockUSDT.address))
      ...
      .withArgs(1, nftAddress, tokenID, owner.address, rentValue, mockUSDT.address)
    ...
    const rentingDetails = await simpleRent.getRentalDetails(1)
  ...
  it("should allow renter to rent", async () => {
    {
      ...
      // Rent the NFT
      await expect(await simpleRent.connect(renter).rent(1, rentDuration))
        .to.emit(simpleRent, "Rented")
        .withArgs(1, nftAddress, tokenID, renter.address, totalRentAmount, rentDuration)
