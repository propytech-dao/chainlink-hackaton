...
task("deploy-checkin", "Deploys the SimpleRentCheckin contract")
  .addParam("simpleRentAddress", "address of previously deployed SimpleRent contract")
  .setAction(async (taskArgs) => {
    console.log(`Deploying checkin contract to  ${network.name}`)
    ...
    const simpleRent = taskArgs.simpleRentAddress

    const consumerContractFactory = await ethers.getContractFactory("SimpleRentCheckin")
    const consumerContract = await consumerContractFactory.deploy(functionsRouter, donIdBytes32, simpleRent, overrides)
    ...
    console.log(`\n deployed SimpleRentCheckin ${consumerContract.address} on ${network.name}`)
  })
